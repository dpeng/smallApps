...
const CString CLunar::m_TIANGAN[]={ //天干
	_T("甲"),_T("乙"),_T("丙"),_T("丁"),_T("戊"),_T("己"),_T("庚"),_T("辛"),
	_T("壬"),_T("癸")
};

const CString CLunar::m_DIZHI[]={ //地支
...
//const CString CLunar::m_cMonName[]  = {"*","正","二","三","四","五","六", "七","八","九","十","十一","腊"};//农历月的名称
...
const CString CLunar::m_wFtv[]={//某月的第几个星期几。 6,7,8,9,10 表示到数第 1,2,3,4,5 个星期几
	/*_T("0120 黑人日"),
	_T("0160 国际麻风节(世界防治麻风病日)[1954]"), //一月的最后一个星期日（月倒数第一个星期日）
	_T("0530 国际母亲节"),
	_T("0540 全国助残日"),
	_T("0640 父亲节"),
	_T("0921 劳动节"),
...
某个小寒点 (1900,0,6,2,5)
*/
int CLunar::getSTerm(int y,int n)//某年的第n个节气为几日(从0小寒起算)
{ 
...
void CLunar::LunarCalendar(COleDateTime tmMeas)//输入阳历日期，返回阴历日期
{
	Init();
	m_tmMeas = tmMeas;
	int i, leap=0, temp=0;
