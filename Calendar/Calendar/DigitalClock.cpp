...
void CDigitalClock::SetClock(int nHour, int nMinute, int nSecond, int nMillisecond)
{
	m_nHour = nHour;
	m_nMinute = nMinute;
	m_nSecond = nSecond;
...
void CDigitalClock::DrawmMillisecond()
{

	int nLeft=m_nXmargin+9*m_nWidth+3*m_nSpace;
	if (m_nMillisecond<10)
	{
		DrawSingleNumber(0,nLeft);
		nLeft+=(int)(1.4*m_nWidth);
		DrawSingleNumber(0,nLeft);
		nLeft+=(int)(1.4*m_nWidth);
		DrawSingleNumber(m_nMillisecond,nLeft);
	}
	else if (m_nMillisecond<100)
	{
		TCHAR c[10];
		_itoa(m_nMillisecond,c,10);
		int num1=c[0]-48;
		int num2=c[1]-48;
		DrawSingleNumber(0,nLeft);
		nLeft+=(int)(1.4*m_nWidth);
...
{	
	...
	{	
		...
		{			
			m_nXmargin = m_rect.Width()-11*m_nWidth-4*m_nSpace-m_nYmargin;
		}
		else
		{
			m_nXmargin = (int)(m_rect.Width()-11*m_nWidth-4*m_nSpace-m_nYmargin)/4;
		}
