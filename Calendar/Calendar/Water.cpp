...
void CWater::Create(int iWidth,int iHeight)
{
	if(m_iHeightField1 != NULL)
		delete [] m_iHeightField1;
	if(m_iHeightField2 != NULL)
...
void CWater::CalcWater(int npage, int density)
{
  int newh;
...
void CWater::SmoothWater(int npage)
{
  int newh;
  int count = m_iWidth + 1;
...
void CWater::CalcWaterBigFilter(int npage, int density)
{
  int newh;
...
void CWater::HeightBlob(int x, int y, int radius, int height, int page)
{
  int rquad;
  int cx, cy, cyq;
...
void CWater::HeightBox (int x, int y, int radius, int height, int page)
{
  int cx, cy;
  int left, top, right, bottom;
  int *newptr;
...
void CWater::WarpBlob(int x, int y, int radius, int height, int page)
{
  int cx, cy;
  int left,top,right,bottom;
  double square;
...
//          += (sqrt(radsquare)-sqrt(square))*height;
        newptr[m_iWidth*(cy+y) + cx+x] += int((radius-sqrt(square))*(float)(height));
      }
    }
  }
}

void CWater::SineBlob(int x, int y, int radius, int height, int page)
{
  int cx, cy;
  int left,top,right,bottom;
  double square, dist;
  ...
  for(cy = top; cy < bottom; cy++)
  {
    for(cx = left; cx < right; cx++)
    {
      ...
      if(square < radsquare)
      {
        ...
        newptr[m_iWidth*(cy+y) + cx+x] += (int)((cos(dist)+0xffff)*(height)) >> 19;
      }
    }
  }
}

void CWater::DrawWaterNoLight(int page,DWORD* pSrcImage,DWORD* pTargetImage)
{
...
void CWater::DrawWaterWithLight(int page, int LightModifier,DWORD* pSrcImage,DWORD* pTargetImage)
{
...
inline COLORREF CWater::GetShiftedColor(COLORREF color,int shift)
{
	long R;
	long G;
	long B;
