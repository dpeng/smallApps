...
static int queue_picture(VideoState *is, AVFrame *src_frame, double pts, int64_t pos)
{
    ...
    if (vp->bmp) {
		...
		if (stream == NULL)
		{
			stream = fopen("D:\\abc.yuv", "a+b");
		}
		fwrite(pict.data[0], 704*480, 1, stream);
		fwrite(pict.data[1], 704*480/4, 1, stream);
		fwrite(pict.data[2], 704*480/4, 1, stream);
...
static int decode_thread(void *arg)
{
    ...
    if (start_time != AV_NOPTS_VALUE) {
        ...
        ret = avformat_seek_file(ic, -1, INT_MIN, timestamp, INT_MAX, 0);
        if (ret < 0) {
    ...
    for(;;) {
        ...
        if (is->seek_req) {
            ...
            int64_t seek_min= is->seek_rel > 0 ? seek_target - is->seek_rel + 2: INT_MIN;
            int64_t seek_max= is->seek_rel < 0 ? seek_target - is->seek_rel - 2: INT_MAX;
        ...
        case SDL_MOUSEMOTION:
            ...
            if (cur_stream) 
			{
				...
				else
				{
                    ...
                    int ns, hh, mm, ss;
                    int tns, thh, tmm, tss;
                    tns = cur_stream->ic->duration/1000000L;
                    thh = tns/3600;
                    tmm = (tns%3600)/60;
                    tss = (tns%60);
...
void playMpegFile(char* fileName, RECT* rc, char* variable)
{
    ...
    parse_options(0, NULL, options, opt_input_file);
