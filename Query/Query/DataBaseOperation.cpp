...
CDataBaseOperation::CDataBaseOperation(void)
{
}
...
void CDataBaseOperation::OpenAdo(char *dbName)
{
...
void CDataBaseOperation::OpenTable(char* tbName)
{
...
void CDataBaseOperation::ReadAdo(char* itemName, char* SearchContent, ItemInfo &item, HWND hWnd)
{
	//CString strName;
	//try
	//{
...
void CDataBaseOperation::AddNew(ItemInfo item)
{
	//try
	//{
	//	m_pRecordset->AddNew();
...
void CDataBaseOperation::ModifyAdo(long long index, ItemInfo item)
{
	//try
	//{
	//	m_pRecordset->MoveFirst();
...
void CDataBaseOperation::DeleteAdo(long long index)
{
	try
	{
		m_pRecordset->MoveFirst();
...
void CDataBaseOperation::ClearAdo(char* tbName)
{
	m_pRecordset.CreateInstance(__uuidof(Recordset));
	char filename[512];
	memset(filename, 0, sizeof(filename));
...
void CDataBaseOperation::CloseTable()
{
	m_pRecordset->Close();
	m_pRecordset = NULL;
}
void CDataBaseOperation::CloseAdo()
{
